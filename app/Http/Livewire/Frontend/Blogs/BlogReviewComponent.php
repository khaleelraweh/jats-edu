...
class BlogReviewComponent extends Component
{
    use LivewireAlert;
    ...
    public function mount()
    {
        ...
        $this->ratingCounts = Review::where('reviewable_id', $this->postId)
    ...
    public function render()
    {
        ...
        $event = Post::with('reviews')->find($this->postId);
        ...
        return view('livewire.frontend.blogs.blog-review-component', compact('reviews', 'averageRating'));
    }
    public function storeReview()
    {
        ...
        $existingReview = Review::where('user_id', auth()->user()->id)
            ->where('reviewable_id', $this->postId)
            ->where('reviewable_type', Post::class)
            ->first();

        if ($existingReview) {
            session()->flash('review_error', 'You have already submitted a review for this post.');
            return;
        }
